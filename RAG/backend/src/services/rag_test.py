...
class RAGAgent:
    """
    RAGAgent는 문서들로부터 RAG 에이전트를 생성합니다.
    ...
    def __init__(
        ...
        embedding_model: Optional[HuggingFaceEmbeddings] = Config.get_embedding_model(),
        ...
        vector_store_path: Optional[str] = Config.vector_store_path
    ):
        # 기본 설정
    ...
    def load_vector_store(self, path: str):
        ...
        Returns:
            None
        """
        self.vector_store = self.vector_store_class.load_local(path, self.embedding_model)
        self._initialize_rag_chain()
...
def create_rag_agent_from_vector_store(
    vector_store_path: str,
    embedding_model: Optional[HuggingFaceEmbeddings] = None,
    **kwargs
) -> RAGAgent:
    """기존 벡터 저장소로부터 RAG 에이전트를 생성합니다.
    
    Returns:
        RAGAgent: RAG 에이전트
    """
    return RAGAgent(vector_store_path=vector_store_path, embedding_model=embedding_model, **kwargs)